...
public class SpringbootAsyncApplication implements CommandLineRunner {
	...
	public void run(String... args) throws Exception {
		...
		List<CompletableFuture<User>> dummys = new ArrayList<CompletableFuture<User>>();
		for (int i = 0 ; i < 4  ; i++) {
			CompletableFuture<User> page1 = new CompletableFuture<User>();
			if(i == 0 ) {
				page1 = gitHubLookupService.findUser("PivotalSoftware");
				dummys.add(page1);
			}
			if(i == 1 ) {
				page1 = gitHubLookupService.findUser("CloudFoundry");
				dummys.add(page1);
			}
			if(i == 2 ) {
				page1 = gitHubLookupService.findUser("RameshMF");
				dummys.add(page1);
			}	
			if(i == 3 ) {
				page1 = gitHubLookupService.findUser("Spring-Projects");
				dummys.add(page1);
			}				
		}
//		CompletableFuture.allOf(cfs).join();
		logger.info("Elapsed time: " + (System.currentTimeMillis() - start));
        CompletableFuture<Void> allOf = CompletableFuture.allOf(dummys.toArray(new CompletableFuture[dummys.size()]));
        allOf.join();
		logger.info("Elapsed time: " + (System.currentTimeMillis() - start));
